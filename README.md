...
tests/conftest.py-> creates a test version of the app and a fake browser client to check if the API works

tests/test_data.py-> uses automates testing framework called pytest that runs all test by itself each function sends request to your API and assets the response matches what's expected if it doesn't pytest automatically reports it as a failure
